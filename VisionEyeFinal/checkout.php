...
if(isset($_POST['order'])){
   ...
   if($cart_total == 0){
      ...
      if($cart_query->rowCount() > 0){
         while($cart_item = $cart_query->fetch(PDO::FETCH_ASSOC)){
            $insert_item = $conn->prepare("INSERT INTO `order_items`(order_id, product_id, price, quantity) VALUES(?,?,?,?)");
            $insert_item->execute([$order_id, $cart_item['pid'], $cart_item['price'], $cart_item['quantity']]);
         }
      }
      // Kosongkan keranjang
